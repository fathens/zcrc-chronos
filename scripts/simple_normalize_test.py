...
def test_simple_pattern():
    ...
    try:
        ...
        norm_variance = sum((x - norm_mean) ** 2 for x in norm_values) / len(norm_values)
    ...
    try:
        ...
        for method in methods:
            try:
                ...
                print(f"\n{method}補間:")
                print(f"  結果: {[round(x, 2) for x in norm_values[:8]]}...")  # 最初の8個だけ表示
                print(f"  データ点数: {len(norm_values)}")
                
                # 変動の保持度
                orig_std = (sum((x - sum(values)/len(values))**2 for x in values) / len(values))**0.5
                norm_mean = sum(norm_values) / len(norm_values)
                norm_std = (sum((x - norm_mean)**2 for x in norm_values) / len(norm_values))**0.5
