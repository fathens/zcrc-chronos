...
def test_specific_edge_cases():
    ...
    edge1_timestamps = [base_time, base_time + datetime.timedelta(seconds=1)]  # 1秒差
...
def test_and_analyze(test_name, timestamps, values):
    ...
    try:
        ...
        if len(timestamps) <= 1:
            print("    データポイントが1以下のため、正規化をスキップする可能性")
        ...
        print(
            f"  実際の結果: {len(values)} → {len(norm_values)}ポイント "
            f"(増加率: {increase_ratio:.1f}x)"
        )
        ...
        if increase_ratio >= 1.8:  # 1.8倍以上を「2倍増加」とみなす
            ...
            if len(norm_timestamps) > 1:
                ...
                print(
                    f"    間隔の一様性: {min(intervals):.3f} - {max(intervals):.3f}秒"
                )
