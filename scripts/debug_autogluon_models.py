...
def debug_autogluon_model_selection():
    ...
    try:
        ...
        # 元データとの比較
        orig_avg_change = sum(abs(values[i+1] - values[i]) for i in range(len(values)-1)) / (len(values)-1)
        pred_avg_change = sum(abs(result[i+1] - result[i]) for i in range(len(result)-1)) / (len(result)-1) if len(result) > 1 else 0
        
        print(f"\n変動性比較:")
        print(f"元データ平均変化: {orig_avg_change:.2f}")
        print(f"予測データ平均変化: {pred_avg_change:.2f}")
        print(f"変動保持率: {pred_avg_change / orig_avg_change:.2%}" if orig_avg_change > 0 else "計算不可")
...
def main():
    ...
    # 1. 基本的なモデル選択デバッグ
    result = debug_autogluon_model_selection()
