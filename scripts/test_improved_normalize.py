    ...
    try:
        ...
        for method in methods_to_test:
            try:
                ...
                print(f"  {method}: 変動保持率={range_retention:.2%}, "
...
def test_outlier_detection_improvement():
    ...
    for i, values in enumerate(normal_price_variations):
        ...
        timestamps = [base_time + datetime.timedelta(hours=j) for j in range(len(values))]
        ...
        try:
            ...
            # 従来基準
            old_outliers = [v for v in values if v < q1 - 1.5*iqr or v > q3 + 1.5*iqr]
            # 新基準
            new_outliers = [v for v in values if v < q1 - 3.0*iqr or v > q3 + 3.0*iqr]
