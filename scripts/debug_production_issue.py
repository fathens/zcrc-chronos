...
def debug_current_implementation():
    ...
    try:
        ...
        # 実際のパターンを模擬
        price_pattern = [
            39000, 40000, 39500, 37000, 37500, 38000, 39000, 39500,  # 初期変動
            40000, 41000, 41500, 42000, 42500, 43000, 43200,        # 上昇
            42800, 42000, 41000, 40000, 38000, 36500, 36000, 36200  # 下降
        ]
...
def check_model_exclusion():
    ...
    try:
        ...
        # 直線性の強いチェック
        if len(pred_values) > 1:
            all_same = all(abs(pred_values[i] - pred_values[0]) < 1 for i in range(len(pred_values)))
            if all_same:
