...
SKIP_INTEGRATION_TESTS = os.environ.get("SKIP_INTEGRATION_TESTS", "").lower() in (
    "true",
    "1",
    "yes",
    "y",
...
def generate_test_data(pattern="uptrend", length=24, noise_level=0.1):
    ...
    if pattern == "uptrend":
        ...
        # 外れ値を含むデータ
        values = [
            20.0 + i * 0.1 + 2.0 * math.sin(i * math.pi / 12) for i in range(length)
        ]
        # 特定の位置に外れ値を挿入
...
def test_real_library_zero_shot_predict():
    ...
    try:
        ...
        print(
            f"予測結果: 長さ={len(forecast_values)}, 平均値={np.mean(forecast_values):.2f}"
        )
...
def test_real_library_with_different_patterns():
    ...
    try:
        ...
        for pattern in patterns:
            ...
            print(f"\nパターン: {pattern}")
            print(
                f"入力データ: 平均={result['input_mean']:.2f}, "
                f"標準偏差={result['input_std']:.2f}"
            )
            print(
                f"予測結果: 平均={result['forecast_mean']:.2f}, "
                f"標準偏差={result['forecast_std']:.2f}"
            )
        ...
        if (
            np.abs(
                results["uptrend"]["forecast_mean"]
                - results["downtrend"]["forecast_mean"]
            )
            < 0.1
        ):
