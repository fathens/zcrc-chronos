...
def test_prediction_horizon_fix():
    ...
    try:
        ...
        if len(result) > 1:
            # 予測の変動性を確認
            pred_changes = [abs(result[i+1] - result[i]) for i in range(len(result)-1)]
            ...
            # 元データとの比較
            orig_changes = [abs(values[i+1] - values[i]) for i in range(len(values)-1)]
...
def test_different_horizons():
    ...
    for horizon in horizons:
        ...
        try:
            ...
            base_time = datetime.datetime(2025, 6, 8, 0, 0, 0)
            timestamps = [base_time + datetime.timedelta(hours=i) for i in range(data_size)]
            ...
            if retention_rate >= 0.5:  # 50%以上保持
                print(f"  ✅ 予測期間が適切に保持されました")
            else:
                print(f"  ⚠️  予測期間が大幅に削減されました")
...
def main():
    ...
    # 1. 予測期間制限の修正テスト
    result = test_prediction_horizon_fix()
