...
def test_normalization(test_name, timestamps, values):
    ...
    try:
        ...
        if increase_ratio > 1.5:
            ...
            original_avg_interval = total_duration / (len(timestamps) - 1) if len(timestamps) > 1 else 0
            
            if len(norm_timestamps) > 1:
                norm_intervals = [(norm_timestamps[i+1] - norm_timestamps[i]).total_seconds() 
...
def test_realistic_trading_scenario():
    ...
    print(f"時間範囲: {(hft_timestamps[-1] - hft_timestamps[0]).total_seconds()/3600:.1f}時間")
