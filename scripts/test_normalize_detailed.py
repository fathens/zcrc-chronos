...
def create_test_data_with_patterns():
    ...
    trend_values = []
    for i in range(20):
        trend_timestamps.append(
            base_time + datetime.timedelta(hours=i * 2 + random.uniform(-0.5, 0.5))
        )
    ...
    cyclic_values = []
    for i in range(20):
        cyclic_timestamps.append(
            base_time + datetime.timedelta(hours=i * 3 + random.uniform(-1, 1))
        )
        # 24時間周期の正弦波 + ノイズ
        cyclic_values.append(
            50 + 20 * math.sin(2 * math.pi * i / 8) + random.uniform(-3, 3)
        )
    ...
    for i in range(20):
        spike_timestamps.append(
            base_time + datetime.timedelta(hours=i * 1.5 + random.uniform(-0.3, 0.3))
        )
        if i == 10:  # 中間で急激な上昇
...
def analyze_pattern_preservation(original_values, normalized_values, pattern_name):
    ...
    orig_std = math.sqrt(
        sum((x - orig_mean) ** 2 for x in original_values) / len(original_values)
    )
    norm_mean = sum(normalized_values) / len(normalized_values)
    norm_std = math.sqrt(
        sum((x - norm_mean) ** 2 for x in normalized_values) / len(normalized_values)
    )
    ...
    if len(original_values) == len(normalized_values):
        ...
        print(
            f"データ長が異なるため相関計算をスキップ ({len(original_values)} vs {len(normalized_values)})"
        )
        correlation = None
    ...
    if len(original_values) > 1 and len(normalized_values) > 1:
        orig_diff = [
            original_values[i + 1] - original_values[i]
        ...
        if len(orig_diff) == len(norm_diff):
            ...
            print(
                f"差分データ長が異なるためトレンド相関計算をスキップ ({len(orig_diff)} vs {len(norm_diff)})"
            )
    ...
    return {
        ...
        "trend_correlation": (
            trend_correlation if "trend_correlation" in locals() else None
        ),
    }
...
def main():
    ...
    for pattern_name, (timestamps, values) in test_patterns.items():
        ...
        if (
            analysis["trend_correlation"] is not None
            and analysis["trend_correlation"] < 0.7
        ):
