...
def test_predictor_initialization():
    ...
    predictor = TimeSeriesPredictor(
        model_name="custom_model", model_params=custom_params
    )
...
def test_zero_shot_predict_different_data_patterns():
    ...
    try:
        for timestamps, values, pattern_name in data_patterns:
            ...
            ), f"{pattern_name}のメタデータにmodel_nameがありません"
            assert (
                "training_samples" in metadata
            ), f"{pattern_name}のメタデータにtraining_samplesがありません"
            assert metadata["training_samples"] == len(
                values
            ), f"{pattern_name}のtraining_samplesが正しくありません"

            # 予測開始時刻の検証（最新の入力データ時刻の次の時間から始まること）
            latest_timestamp = max(timestamps)
            assert (
                forecast_timestamps[0] > latest_timestamp
            ), f"{pattern_name}の予測開始時刻が最新の入力データ時刻より後ではありません"

            # 予測タイムスタンプの連続性をチェック
            for i in range(1, len(forecast_timestamps)):
                diff_seconds = (
                    forecast_timestamps[i] - forecast_timestamps[i - 1]
                ).total_seconds()
                assert (
                    diff_seconds > 0
                ), f"{pattern_name}の予測タイムスタンプが正しく並んでいません"
...
def test_zero_shot_predict_with_patterns():
    ...
    try:
        ...
        for values, pattern_name in patterns:
            ...
            assert all(
                np.isfinite(v) for v in forecast_values
            ), f"{pattern_name}の予測値に無効な値が含まれています"
            ...
            # 予測統計値のログを記録
            print(
                f"{pattern_name} - 入力: 平均 {input_mean:.2f}, 標準偏差 {input_std:.2f}"
            )
            print(
                f"{pattern_name} - 予測: 平均 {forecast_mean:.2f}, 標準偏差 {forecast_std:.2f}"
            )

            # 予測値の標準偏差が入力データより極端に大きくないことを確認
            assert (
                forecast_std < input_std * 10
            ), f"{pattern_name}の予測値の標準偏差が極端に大きすぎます"
...
def test_zero_shot_predict_with_outliers():
    ...
    try:
        ...
        print(
            f"外れ値を含むデータの予測結果 - 平均: {forecast_mean:.2f}, 標準偏差: {forecast_std:.2f}"
        )
