...
def test_different_data_scenarios():
    ...
    scenarios = [
        {"name": "単調増加データ", "data": [36000 + i * 200 for i in range(20)]},
        {
            "name": "高変動データ",
        ...
        },
        {
            "name": "ランダムウォーク",
            "data": [36000]
            + [36000 + sum((j % 7 - 3) * 100 for j in range(i + 1)) for i in range(19)],
        },
    ]
    ...
    for scenario in scenarios:
        ...
        try:
            ...
            timestamps = [
                ...
                for i in range(len(scenario["data"]))
            ]

            predictor = TimeSeriesPredictor()

            pred_timestamps, pred_values, metadata = predictor.zero_shot_predict(
                timestamp=timestamps, values=scenario["data"], horizon=12
            )

            # 変動性分析
            if len(pred_values) > 1:
                pred_var = sum(
                    (pred_values[i + 1] - pred_values[i]) ** 2
                    for i in range(len(pred_values) - 1)
                )
                orig_var = sum(
                    (scenario["data"][i + 1] - scenario["data"][i]) ** 2
                    for i in range(len(scenario["data"]) - 1)
                )
